...
statistic dependent on $W$ and $A$, but, between $i$, the
probabilities are depedent.  The $R$th message time can be easily
determined (see Section~\ref{sec:dynamo-latency}), but a concise,
closed form for the probability of inconsistency is highly complicated
and eludes us.  Dynamo is simple to reason about and program, but
difficult to analyze in closed form.  As we discuss in
Section~\ref{sec:sim}, we explore $WARS$ using Monte Carlo analysis.


\subsection{Dynamo-style Operation Latency}

\label{sec:dynamo-latency}
