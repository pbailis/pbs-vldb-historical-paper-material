...
or even across datacenters for two reasons: first, to provide 
...
In order to provide predictable read and write latency, systems often
choose to eschew consistency across replicas and instead provide
...
achieve favorable performance and handle intermittent failures, 
...
``Probabilistic quorums'' have been previously studied in a 
theoretical context to provide arbitrarily high probability of strong
...
In this paper we present algorithms and models for accurately predicting
the staleness of a data item across multiple versions and
wall clock time. This can be used to answer, with high probability,
queries of the form: fetch a data item within $k$ versions of the 
latest committed version or read a data item at most $t$ seconds older
than the last committed version.  We also use these consistency measures 
to provide probabilistic guarantees on monotonic reads, a form of 
session guarantees where reads are guaranteed to return data items 
no older than what has been previously read.

By relaxing the consistency guarantees from strong consistency to a 
bounded-staleness model and by modeling how the anti-entropic 
process across replicas over time, we provide exponential improvements 
in the probability of returning staler-than-promised data. 
This results in techniques that are useful at replication levels as 
seen in practice ($n<10$, and easily $n\leq3$) and provides a 
consistency-latency trade-off as shown in Figure~\ref{fig:latency-staleness}.

Using this theory, we have developed 'Ernst' a replica management policy
that determines the read and write quorum sizes ($w$,$r$,$n$) to
optimize the performance of an operation given the desired staleness
bound. We implement 'Ernst' on top of Cassandra, a widely-deployed 
open source quorum-based distributed storage system, and demonstrate how
it can be easily applied in practice. We validate our new theory using 
several microbenchmarks and synthetic workloads and then demonstrate 
the utility of staleness measures in a Twitter-clone with several thousand clients.
...
quorum will either have been committed earlier than $t$ units of time
before the read began (but may not be committed yet).
\end{definition}
