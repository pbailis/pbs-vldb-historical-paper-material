...
for config in configs:
    if config.name != "YMMR":
        continue
    results[config] = []
    for writespacing in writespaces:
        ts = []
        stales = []
        ...
        plot(ts, stales, config.markerfmt, label=str(writespacing))
...
legend(loc="lower right", title="Time Between Writes")
savefig("writespeed.pdf")
show()
